...
% 
[ClsFtrs, CtcFtrs, FreFtrs, ClsTims, CtcTims, FreTims] = GenFtr(GenList(RtPath), ...
                            SetSegConf(eye(7)', 1024, 256, @rectwin, [0.4, 1]), ...
                            SetFtrConf("rnn"), ...
                            SetFltConf("nofilter", 100), ...
                            RtPath, "3D");
...
Sz = size(ClsFtrs);
for i = 1:Sz(1)
    item = reshape(ClsFtrs(i,:,:), Sz(2:3));
    writematrix(item, append('cls-joint-#',num2str(i),'.csv'));
end

Sz = size(CtcFtrs);
for i = 1:Sz(1)
    item = reshape(CtcFtrs(i,:,:), Sz(2:3));
    writematrix(item, append('ctc-joint-#',num2str(i),'.csv'));
end

Sz = size(FreFtrs);
for i = 1:Sz(1)
    item = reshape(FreFtrs(i,:,:), Sz(2:3));
    writematrix(item, append('fre-joint-#',num2str(i),'.csv'));
end